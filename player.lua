...
function Player:setPosition(x, y)
    ...
    self.eyes[1].x = self.x
    self.eyes[1].y = self.y

    self.eyes[2].x = self.x
    self.eyes[2].y = self.y
end
...
function Player:drawPlayer(hitbox, map)

    --map is parsed to get a list of collisions so the players mesh can change based on touch
    self.visual:drawExpandingCircle(self.x, self.y, 25.5, 1, 1, 16, "fill", 1, 6,14,81, map)
    self:drawEyes(map)
...
function Player:drawEyes(map)
    
    local s = self.eyes

    self.visual:drawExpandingCircle(s[1].x, s[1].y, s[1].size, 0.5, 2, 16, "fill", 1, 230, 230, 230, map)
    self.visual:drawExpandingCircle(s[2].x, s[2].y, s[2].size, 0.5, 2, 16, "fill", 1,  230, 230, 230, map)
